...
func TestExample(t *testing.T) {
	docs, err := parseInput(testInput)
	require.NoError(t, err)
	assert.Equal(t, 2, countValid(docs))
}
...
func TestInvalidPassports(t *testing.T) {
	docs, err := parseInput(invalidPassports)
	require.NoError(t, err)
	assert.Len(t, docs, 4)
	assert.Equal(t, 0, countValid2(docs))
}
...
func TestValidPassports(t *testing.T) {
	docs, err := parseInput(validPassports)
	require.NoError(t, err)
	assert.Len(t, docs, 4)
	assert.Equal(t, 4, countValid2(docs))
}
